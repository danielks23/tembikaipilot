...
class ModelState:
  ...
  def __init__(self, context: CLContext):
    ...
    self.inputs = {
       ...
       'nav_features': np.zeros(256, dtype=np.float32),  # kept for model compatibility, always zero
       'nav_instructions': np.zeros(150, dtype=np.float32),  # kept for model compatibility, always zero
...
def main(demo=False):
  ...
  sm = SubMaster(["carState", "roadCameraState", "liveCalibration", "driverMonitoringState", "carControl"])
  ...
  while True:
    ...
    # Nav features disabled — no navd/navmodeld on KA2
    timestamp_llk = 0
    nav_enabled = False
    nav_features[:] = 0
    nav_instructions[:] = 0
    ...
    if model_output is not None:
      ...
      fill_model_msg(modelv2_send, model_output, publish_state, meta_main.frame_id, meta_extra.frame_id, frame_id, frame_drop_ratio,
                       meta_main.timestamp_eof, timestamp_llk, model_execution_time, False, live_calib_seen)
