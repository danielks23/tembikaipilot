#!/usr/bin/env python3
# GPS daemon for Quectel EC25-EM LTE modem (integrated GNSS).
# Reads raw GNSS measurements via the modem's diag protocol on /dev/ttyUSB0.
import os
import sys
import signal
import itertools
...
from openpilot.system.gpsd.modemdiag import ModemDiag, DIAG_LOG_F, setup_logs, send_recv
from openpilot.system.gpsd.structs import (dict_unpacker, position_report, relist,
...
ASSIST_DATA_CACHE = '/data/xtra3grc.bin'
ASSISTANCE_URLS = [
...
def at_cmd(cmd: str) -> str | None:
  return subprocess.check_output(f"mmcli -m any --timeout 5 --command='{cmd}'", shell=True, encoding='utf8')

def gps_enabled() -> bool:
  return "QGPS: 1" in at_cmd("AT+QGPS?")

def download_assistance():
  for url in ASSISTANCE_URLS:
    try:
      ...
      try:
        shutil.copy2(ASSIST_DATA_FILE, ASSIST_DATA_CACHE)
      except Exception:
        pass
      cloudlog.info(f"Downloaded assistance data from {url}")
      return

    except requests.exceptions.RequestException:
      continue
  cloudlog.error("Failed to download assistance file from all sources")
  return
...
@retry(attempts=5, delay=0.2, ignore_failure=True, log_traceback=False)
def inject_assistance():
  cmd = f"mmcli -m any --timeout 5 --location-inject-assistance-data={ASSIST_DATA_FILE}"
...
def setup_quectel(diag: ModemDiag) -> bool:
  ...
  # Enable SUPL network assistance (modem fetches directly over cellular)
  at_cmd("AT+QGPSXTRA=1")
  at_cmd("AT+QGPSSUPLURL=\"supl.google.com\"")
  # Try fresh file first, then cached file
  if os.path.exists(ASSIST_DATA_FILE):
    ret = True
    inject_assistance()
    os.remove(ASSIST_DATA_FILE)
  elif os.path.exists(ASSIST_DATA_CACHE):
    try:
      shutil.copy2(ASSIST_DATA_CACHE, ASSIST_DATA_FILE)
      ret = True
      inject_assistance()
      os.remove(ASSIST_DATA_FILE)
    except Exception:
...
def wait_for_modem():
  ...
  consecutive_ok = 0
  while True:
    ret = subprocess.call("mmcli -m any --timeout 5 --command=\"AT+QGPS?\"", stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, shell=True)
    if ret == 0:
      consecutive_ok += 1
      if consecutive_ok >= 3:
        return
